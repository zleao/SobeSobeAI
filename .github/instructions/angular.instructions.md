...
- When using Angular >= 17.2 (where signal-based input/output and query functions are stable), use `input()`, `output()`, `viewChild()`, `viewChildren()`, `contentChild()` and `contentChildren()` functions instead of decorators; for earlier Angular versions, use the corresponding decorators (e.g., `@Input()`, `@Output()`, `@ViewChild()`, `@ViewChildren()`, `@ContentChild()`, `@ContentChildren()`)
